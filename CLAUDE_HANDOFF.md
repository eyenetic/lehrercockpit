...
> Last updated: 2026-03-31 (Phase 18 — Orgaplan live-PDF + Heute/Woche Darstellung)
>
...
- **Phase 18 — Orgaplan live-PDF (HES) + Heute/Woche Darstellung + Datum-Bug-Fix:**
  - `backend/plan_digest.py`: `_berlin_today()` (zoneinfo + UTC+2-Fallback); `_serialize_entry()` liefert `isoDate`; `upcoming` enthält alle zukünftigen Einträge ohne Limit
  - `backend/api/dashboard_routes.py`: HES-PDF-URL `https://hermann-ehlers-schule.de/wp-content/uploads/2026/03/Orgaplan-2025_26-ab-April.pdf` als Fallback; `_slice_by_date()` berechnet `today_entries`/`week_entries` bei jedem Request frisch aus Berliner Lokalzeit (nie aus Cache)
  - `src/features/classwork.js`: `renderOrgaplanTodayCard()` (Heute-Karte mit Allgemein/Mittelstufe/Oberstufe), `renderOrgaplanWeekSection()` (nach Datum gruppierte Wochenübersicht); `renderPlanDigest()` befüllt `#orgaplan-today-list` und `#orgaplan-week-list`
  - `index.html` (v=53): Orgaplan-Block hat drei Ebenen — **Heute** (`#orgaplan-today-list`), **Diese Woche** (`#orgaplan-week-list`), **Alle weiteren Einträge** (`<details>` eingeklappt)
  - `src/app.js`: `_applyOrgaplanV2Data()` überträgt `today_entries`/`week_entries`; `pickOrgaplanBriefing()` bevorzugt `today_entries[0]`; `renderSectionFocus()` versteckt Divider per `dataset.dividerFor` statt blind alle
  - `styles.css`: neue Klassen für today/week-Blöcke
- **Phase 17 — `app.js` dead-code cleanup pass** (−104 lines removed, +79 net): (1) Removed shadowed duplicate `weekdayLabel` function (overridden by real version further down); (2) Trimmed 28-line itslearning connector fallback body — now a 2-line delegation wrapper since `LehrerItslearning` is verified; (3) Trimmed all 5 WebUntis `TODO: remove` fallback bodies (`renderWebUntisControls`, `renderWebUntisWatchlist`, `renderWebUntisSchedule`, `normalizeLocalWebUntisState`, `compactEventDetail`) to clean delegation stubs; (4) Replaced outdated "TODO: Split this file" header comment with accurate orchestrator description listing all 8 extracted modules. 4 new tests added.
